...
@$month=mysqli_fetch_array($conn->query("SELECT 
	...
	<div class="tab-content" id="myTabContent">
		...
		<div class="tab-pane fade" id="summary" role="tabpanel" aria-labelledby="summary-tab">
			<a target="_blank" href="transact-hazard/filter/export-pic.php?d1=<?= $date1;?>&d2=<?= $date2;?>&comp=<?= $comp;?>&div=<?php echo $_SESSION['user_divisi'];?>">
					...
					<div class="col-4 col-sm-4 col-lg-4">
											...
											<tr>
												...
												<td>
													<?php @$classi_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard 
														where hazard_comp = '$comp' 
														and hazard.hazard_divisi = ".$_SESSION['user_divisi']." and hazard_classi = ".$row['classi_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'"));  $classi_g_[] = $classi_['total']; echo $classi_['total'];?> 
													</td>
												</tr>
											<?php } ?>
										</tbody>
												...
												<tr>
													...
													<td>
														<?php @$risk_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard 
															where hazard_comp = '$comp'
															and hazard.hazard_divisi = ".$_SESSION['user_divisi']." and hazard_risk = ".$row['risk_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'")); $risk_g_[] = $risk_['total']; echo $risk_['total'];?>
														</td>
													</tr>
												<?php } ?>
											</tbody>
											...
											<tbody>
												<?php while($row  = mysqli_fetch_array($location)){ 
													...
													<tr>
														<td><?= $row['loc_name']?></td>
														<td>
															<?php @$location_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard where hazard_comp = '$comp'
															and hazard.hazard_divisi = ".$_SESSION['user_divisi']." and  hazard_loc = ".$row['loc_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'")); $location_g_[]= $location_['total']; echo $location_['total'];?>
														</td>
													</tr>
												<?php } ?>
											</tbody>
						...
						<div class="row ">
							<div class="col-12 col-md-12 col-lg-12">
								<div class="card ">
									<div class="card-header">
										<h4>Berdasarkan Divisi PIC</h4><br>
									</div>
									<div class="card-body">
										<div class="table-responsive">
											<table class="table table-striped  table-hover" id="tableExportdivpelsu" style="width:100%;">
												<thead>
													<tr>
														<th>Divisi</th>
														<th hidden style="background-color:cyan;">Pengawas</th>
														<th>JAN</th>
														<th>FEB</th>
														<th>MAR</th>
														<th>APR</th>
												...
												<tbody>
													<?php while($row  = mysqli_fetch_array($divisi)){ 
														...
														$mdiv=mysqli_fetch_array($conn->query("SELECT 
															...
															AND hazard_comp = ".$comp."
															and hazard.hazard_divisi = ".$_SESSION['user_divisi']."
															and hazard_divisi = ".$row['divisi_id'].""));
															?>
															<tr>
																<td>
																	<?= $row['divisi_name']?>
																</td>
																<td hidden style="background-color:cyan;">
