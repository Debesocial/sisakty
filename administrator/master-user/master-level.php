						...
						<div class="card-body">
							<div class="row">
								<div class="col-md-12 col-lg-12 col-xl-12">
									<?php 
									@$id = @$_GET['id'];
									@$levelupdate = mysqli_fetch_array($conn->query("SELECT * FROM level left join company on company.comp_id = level.level_comp where  level.level_id = '$id'"));?>
									<div class="form-group">
										<label>Jabatan</label>
										<input type="text" class="form-control" id="" required placeholder="Jabatan" name="level" 
...
=======
								...
								// 	$delete   = mysqli_query($conn, $sql);
								// 	if ($delete) {
								// 		echo"<script> Swal.fire({icon: 'success', title: 'Data berhasil dihapus', showConfirmButton: false, timer: 1500 }).then(function() { location.href = 'home.php?v=mlevel&act=add'; }); </script>";
								// 	} else { 
								// 		echo"<script>Swal.fire({ icon: 'success', title: 'Data gagal dihapus', showConfirmButton: false, timer: 1500}).then(function() {location.href = 'home.php?v=mlevel&act=add';});</script>";
								// 	}
								// }
								...
								while($row  = mysqli_fetch_array($data)){ ?> 
									<tr>
										...
										<td>
											...
											<!-- <a href="#" data-id="<?= $row['level_id']; ?>" id="del" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>   -->
										</td>
									</tr>
								<?php } ?>
							</tbody>
