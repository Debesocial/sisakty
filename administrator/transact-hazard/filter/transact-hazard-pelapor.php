...
<div class="tab-content" id="myTabContent">
	<div class="alert alert-light" role="alert">
		<center><strong>
				<?php @$tcomp = mysqli_fetch_array($conn->query("SELECT * FROM company where comp_id = '$comp'"));
				echo $tcomp['comp_name'] . ' sebagai ' . $_POST['sebagai']; ?></strong>
			<?php if (@$_POST['date1'] != '') { ?>
										...
										<tr>
											...
											<td>
												<?php @$classi_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
														left join user on user.user_id = hazard.hazard_user
														where user.user_comp = '$comp' and hazard_classi = " . $row['classi_id'] . " and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'"));
										...
										<tr>
											...
											<td>
												<?php @$risk_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
															left join user on user.user_id = hazard.hazard_user
															where user.user_comp = '$comp' and hazard_risk = " . $row['risk_id'] . " and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'"));
												$risk_g_[] = $risk_['total'];
												echo $risk_['total']; ?>
											</td>
										</tr>
										...
										<tr>
											...
											<td>
												<?php @$location_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
																left join user on user.user_id = hazard.hazard_user
																where user.user_comp = '$comp' and hazard_loc = " . $row['loc_id'] . " and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'"));
									...
									<tbody>
										...
										?>
											<tr>
												...
												<td>
													<?php @$pengawas = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM user where user_divisi = " . $row['divisi_id'] . " and user_comp =" . $comp . ""));
													echo $pengawas['total']; ?>
												</td>
												<td>
												...
												<td style="background-color:yellow;">
													<?= $total = $mdiv['jan'] + $mdiv['feb'] + $mdiv['mar'] + $mdiv['apr'] + $mdiv['mei'] + $mdiv['jun'] + $mdiv['jul'] + $mdiv['agu'] + $mdiv['sep'] + $mdiv['okt'] + $mdiv['nov'] + $mdiv['des'];
													$divisi_g_[] = $total; ?>
												</td>
												<td style="background-color:yellow;">
