...
if (@$_GET['act'] == 'detail') {
  @$report = mysqli_fetch_array(
    $conn->query("SELECT * FROM hazard 
      ...
      <div class="card author-box">
        <div class="card-body">
          <div class="author-box-center">
            ...
            <div class="author-box-job"> <?= $report['user_status']; ?> </div>
          </div>
          <hr>
          <div class="row">
            <div class="form-group col-md-4"> <label>NIK</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['user_nik']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Nama Lengkap</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['user_name']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Tanggal Lahir</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['user_birth']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Email</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['user_email']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Masuk Site</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['user_onsite']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Jabatan</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['level_name']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Departement</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['dept_name']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Divisi</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['divisi_name']; ?> </span> </div>
          </div>
          <div class="row">
            <div class="form-group col-md-4"> <label>Perusahaan</label> </div>
            <div class="form-group col-md-6"> <span class="float-left text-muted"> <?= $report['comp_name']; ?> </span> </div>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="card-body">
          <table class="table table-striped">
              ...
              <figure>
                <center>
                  <a href="" data-toggle="modal" data-target=".bd-example-modal-lg">
                    <img class="zoom border rounded" style=" border: 1px solid #ddd;border-radius: 4px;padding: 5px;" src="<?= '../assets/hazard/thumbnail/' . $report['hazard_photo']; ?>" alt="image" class="imaged img-fluid" width="50%">
                  </a>
                </center>
              </figure>
            <?php } else { ?>
            ...
            <tbody>
              <tr>
                <th>ID Harmonis</th>
                <td> <?= 'HZ' . str_pad($report['hazard_id'], 5, "0", STR_PAD_LEFT); ?></td>
              </tr>
              <tr>
                <th>Judul</th>
                <td><?= $report['hazard_name']; ?></td>
              </tr>
              <tr>
                <th>Klasifikasi</th>
                <td><?= $report['classi_name']; ?></td>
              </tr>
              <tr>
                <th>Lokasi</th>
                <td>
              ...
              <tr>
                <th>Uraian</th>
                <td><?= $report['hazard_desc']; ?></td>
              </tr>
              <tr>
                <th>Saran</th>
                <td><?= $report['hazard_solution']; ?></td>
              </tr>
            </tbody>
          </table>
                    ...
                    <tbody>
                      <?php while ($row  = mysqli_fetch_array($classi)) {
                        ...
                        <tr>
                          <td><?= $row['classi_name'] ?></td>
                          <td>
                            <?php @$classi_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard WHERE hazard_classi = " . $row['classi_id'] . " and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'"));
                            $classi_g_[] = $classi_['total'];
                            echo $classi_['total']; ?>
                          </td>
                        </tr>
                      <?php } ?>
                    ...
                    <tbody>
                      <?php while ($row  = mysqli_fetch_array($risk)) {
                        ...
                        <tr>
                          <td><?= $row['risk_name'] ?></td>
                          <td>
                            <?php @$risk_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard WHERE hazard_risk = " . $row['risk_id'] . " and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'"));
                            $risk_g_[] = $risk_['total'];
                            echo $risk_['total']; ?>
                          </td>
                        </tr>
                    ...
                    <tbody>
                      <?php while ($row  = mysqli_fetch_array($location)) {
                        ...
                        <tr>
                          <td><?= $row['loc_name'] ?></td>
                          <td>
                            <?php @$location_ = mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard WHERE hazard_loc = " . $row['loc_id'] . " and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'"));
                            $location_g_[] = $location_['total'];
                            echo $location_['total'] . ''; ?>
                          </td>
                        </tr>
                      <?php } ?>
                    </tbody>
