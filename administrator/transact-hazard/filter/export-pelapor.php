...
$objPHPExcel->getActiveSheet()->setCellValue('B2', 'BERDASARKAN DIVISI PIC');
$objPHPExcel->getActiveSheet()->setCellValue('B3', 'Divisi');
$objPHPExcel->getActiveSheet()->setCellValue('C3', 'Pengawas');
$objPHPExcel->getActiveSheet()->setCellValue('D3', 'Target');
$objPHPExcel->getActiveSheet()->setCellValue('E3', 'JAN');
$objPHPExcel->getActiveSheet()->setCellValue('F3', 'FEB');
$objPHPExcel->getActiveSheet()->setCellValue('G3', 'MAR');
$objPHPExcel->getActiveSheet()->setCellValue('H3', 'APR');
$objPHPExcel->getActiveSheet()->setCellValue('I3', 'MEI');
$objPHPExcel->getActiveSheet()->setCellValue('J3', 'JUN');
$objPHPExcel->getActiveSheet()->setCellValue('K3', 'JUL');
$objPHPExcel->getActiveSheet()->setCellValue('L3', 'AGU');
$objPHPExcel->getActiveSheet()->setCellValue('M3', 'SEP');
$objPHPExcel->getActiveSheet()->setCellValue('N3', 'OKT');
$objPHPExcel->getActiveSheet()->setCellValue('O3', 'NOV');
$objPHPExcel->getActiveSheet()->setCellValue('P3', 'DES');
$objPHPExcel->getActiveSheet()->setCellValue('Q3', 'Total YTD');
$objPHPExcel->getActiveSheet()->setCellValue('R3', 'Target YTD');
$objPHPExcel->getActiveSheet()->setCellValue('S3', 'YTD %');
$divisi   = mysqli_query ($conn,"SELECT * FROM divisi 
	where divisi_comp = '$comp' order by divisi_name asc ");
$row = 4;
while($divisi_data = mysqli_fetch_array($divisi))
{
	$pengawas=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM user 
		where user_divisi = ".$divisi_data['divisi_id'].""));
	$mdiv=mysqli_fetch_array($conn->query("SELECT 
		...
		where DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' 
		AND DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'
		AND user.user_divisi = ".$divisi_data['divisi_id']."
		AND user.user_comp = '$comp'"));

	$objPHPExcel->getActiveSheet()->setCellValue('B'.$row, $divisi_data['divisi_name']);
	$objPHPExcel->getActiveSheet()->setCellValue('C'.$row, $pengawas['total']);
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $pengawas['total'] * 2);
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $mdiv['jan']);
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, $mdiv['feb']);
	$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $mdiv['mar']);
	$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, $mdiv['apr']);
	$objPHPExcel->getActiveSheet()->setCellValue('I'.$row, $mdiv['mei']);
	$objPHPExcel->getActiveSheet()->setCellValue('J'.$row, $mdiv['jun']);
	$objPHPExcel->getActiveSheet()->setCellValue('K'.$row, $mdiv['jul']);
	$objPHPExcel->getActiveSheet()->setCellValue('L'.$row, $mdiv['agu']);
	$objPHPExcel->getActiveSheet()->setCellValue('M'.$row, $mdiv['sep']);
	$objPHPExcel->getActiveSheet()->setCellValue('N'.$row, $mdiv['okt']);
	$objPHPExcel->getActiveSheet()->setCellValue('O'.$row, $mdiv['nov']);
	$objPHPExcel->getActiveSheet()->setCellValue('P'.$row, $mdiv['des']);
	$objPHPExcel->getActiveSheet()->setCellValue('Q'.$row, $total = $mdiv['jan']+$mdiv['feb']+$mdiv['mar']+$mdiv['apr']+$mdiv['mei']+$mdiv['jun']+$mdiv['jul']+$mdiv['agu']+$mdiv['sep']+$mdiv['okt']+$mdiv['nov']+$mdiv['des']);
	$objPHPExcel->getActiveSheet()->setCellValue('R'.$row, $targety = $pengawas['total'] * 24);
	$objPHPExcel->getActiveSheet()->setCellValue('S'.$row, $acv = $total / $targety * 100);
	$row++;
}
//--------------------------------------------------------------------------------------------------
...
$objPHPExcel->getActiveSheet()->setCellValue('D3', 'Open');
$objPHPExcel->getActiveSheet()->setCellValue('E3', 'Progress');
$objPHPExcel->getActiveSheet()->setCellValue('F3', 'Closed');
$objPHPExcel->getActiveSheet()->setCellValue('G3', 'Reject');
...
while($user_data = mysqli_fetch_array($user))
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$row, $muser['Open']);
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$row, $muser['Progress']);
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$row, $muser['Closed']);
	$objPHPExcel->getActiveSheet()->setCellValue('G'.$row, $muser['Reject']);
	$objPHPExcel->getActiveSheet()->setCellValue('H'.$row, $totalusr =  $muser['Open'] +  $muser['Progress'] + $muser['Closed'] + $muser['Reject']);
	$row++;
}
//--------------------------------------------------------------------------------------------------
