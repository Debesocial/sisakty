...
@$month=mysqli_fetch_array($conn->query("SELECT 
	...
	<div class="tab-content" id="myTabContent">
		<div class="alert alert-light" role="alert"> 
			<center><strong>
				<?php @$tcomp = mysqli_fetch_array($conn->query("SELECT * FROM company")); echo $tcomp['comp_name'].' sebagai '.$_POST['sebagai'];?></strong>
				<?php if (@$_POST['date1'] != '') { ?>
		...
		<div class="tab-pane fade" id="summary" role="tabpanel" aria-labelledby="summary-tab">
			<a target="_blank" href="transact-hazard/filter/export-all.php?d1=<?= $date1;?>&d2=<?= $date2;?>">
											...
											<tr>
												...
												<td>
													<?php @$classi_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
														left join user on user.user_id = hazard.hazard_user
														where hazard_classi = ".$row['classi_id']." and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'"));  $classi_g_[] = $classi_['total']; echo $classi_['total'];?> 
													</td>
												</tr>
											<?php } ?>
										</tbody>
												...
												<tr>
													...
													<td>
														<?php @$risk_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
															left join user on user.user_id = hazard.hazard_user
															where hazard_risk = ".$row['risk_id']." and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'")); $risk_g_[] = $risk_['total']; echo $risk_['total'];?>
														</td>
													</tr>
												<?php } ?>
											</tbody>
													...
													<tr>
														...
														<td>
															<?php @$location_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard  
																left join user on user.user_id = hazard.hazard_user
																where hazard_loc = ".$row['loc_id']." and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard.hazard_date, '%Y-%m-%d') <= '$date2'")); $location_g_[]= $location_['total']; echo $location_['total'];?>
															</td>
														</tr>
													<?php } ?>
												</tbody>
