...
@$month=mysqli_fetch_array($conn->query("SELECT 
	...
	<div class="tab-content" id="myTabContent">
		...
		<div class="tab-pane fade" id="summary" role="tabpanel" aria-labelledby="summary-tab">
			<a target="_blank" href="transact-hazard/filter/export-pic.php?d1=<?= $date1;?>&d2=<?= $date2;?>&comp=<?= $comp;?>">
										...
										<?php while($row  = mysqli_fetch_array($classi)){ 
											...
											<tr>
												...
												<td>
													...
													where hazard_comp = '$comp' and hazard_classi = ".$row['classi_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'"));  $classi_g_[] = $classi_['total']; echo $classi_['total'];?> 
												</td>
											</tr>
										<?php } ?>
									</tbody>
										...
										<?php while($row  = mysqli_fetch_array($risk)){ 
											...
											<tr>
												...
												<td>
													...
													where hazard_comp = '$comp' and hazard_risk = ".$row['risk_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'")); $risk_g_[] = $risk_['total']; echo $risk_['total'];?>
												</td>
											</tr>
										<?php } ?>
									</tbody>
									...
									<tbody>
										<?php while($row  = mysqli_fetch_array($location)){ 
											...
											<tr>
												<td><?= $row['loc_name']?></td>
												<td>
													<?php @$location_=mysqli_fetch_array($conn->query("SELECT COUNT(*) AS total FROM hazard where hazard_comp = '$comp' and  hazard_loc = ".$row['loc_id']." and DATE_FORMAT(hazard_date, '%Y-%m-%d') >= '$date1' and DATE_FORMAT(hazard_date, '%Y-%m-%d') <= '$date2'")); $location_g_[]= $location_['total']; echo $location_['total'];?>
												</td>
											</tr>
										<?php } ?>
									</tbody>
